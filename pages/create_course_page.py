...
class CreateCoursePage(BasePage):
    def __init__(self, page: Page):
        ...
        self.create_course_estimated_input = page.get_by_test_id("create-course-form-estimated-time-input").locator(
            "input")
    ...
    def check_visible_create_course_title(self):
        ...
        expect(self.create_course_title).to_have_teext("Create course")

    def click_create_course_button(self):
        expect(self.create_course_button).click()

    def check_visible_create_course_button(self):
        expect(self.create_course_button).to_be_visible()  # у меня тут так а не как у проподавателя
    ...
    def check_visible_image_upload_view(self, is_image_uploaded: bool = False):
        ...
        expect(self.preview_empty_view_title).to_be_visible()
        expect(self.preview_empty_view_title).to_have_text('Tap on "Upload image" button to select file')

        expect(self.preview_empty_view_description).to_be_visible()
        expect(self.preview_empty_view_description).to_have_text("Recommended file size 540X300")
    ...
    def check_visible_create_course_form(
        ...
        expect(self.create_course_min_score_input).to_have_value(description)
    ...
    def click_create_exercises_button(self):
        expect(self.create_exercises_button).click()

    def check_visible_exercises_empty_view(self):
        expect(self.exercises_empty_view_icon).to_be_visible()

        expect(self.exercises_empty_view_title).to_be_visible()
        expect(self.exercises_empty_view_title).to_have_text("There is no exercise")
    ...
    def click_delete_exercises_button(self, index: int):
        delete_exercise_button = self.page.get_by_text_id(
            f"create-course-exercises-{index}-box-toolbar-delete-exercise-button")
        delete_exercise_button.click()

    def check_visible_create_exercises_form(self, index: int, title: str, description: str):
        exercise_subtitle = self.page.get_by_test_id(f"create-course-exercises-{index}-box-toolbar-subtitle-text")
        exercise_title_input = self.page.get_by_test_id(f"create-course-exercises-form-title-{index}-input")
        exercise_description_input = self.page.get_by_test_id(f"create-course-exercises-form-description-{index}-input")
